...
<body>
    ...
    <div id="container">
        <form method="post">
            ...
            <input type="text" placeholder="Entrer le nom d'utilisateur" id="inscription_nom" name="username"> -->
    ...
    if (isset($_POST['inscription_send'])) {
        ...
        if (!empty($inscription_mdp) && !empty($inscription_email) && !empty($inscription_confirmation_mdp)) {
            if ($inscription_mdp == $inscription_confirmation_mdp) {
                ...
                $statement = $db->prepare("SELECT mail FROM utilisateur WHERE email = :email");
